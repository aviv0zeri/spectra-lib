...
export function NotificationBanner({ event, icon, actions, colors, rtl = false, duration = 4000, dismissDirection, onPress, onDismiss, Container, style, titleStyle, bodyStyle, testID, }) {
    ...
    const scheduleAutoDismiss = () => {
        ...
        if (duration === false)
            return;
        const screenExit = direction === 'up' ? -Dimensions.get('window').height : Dimensions.get('window').width;
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [direction]);
    const transform = direction === 'up' ? [{ translateY: translate }] : [{ translateX: translate }];
    return (_jsx(Animated.View, { style: [{ opacity, transform }], ...panResponder.panHandlers, testID: testID, children: _jsxs(Wrapper, { style: [styles.container, { backgroundColor: colors.panel, borderColor: colors.rim }, style], children: [_jsx(Pressable, { onPress: onPress, accessibilityRole: onPress ? 'button' : undefined, children: _jsxs(View, { style: [styles.row, { direction: rtl ? 'rtl' : 'ltr' }], children: [icon != null ? icon : null, _jsxs(View, { style: [styles.column, { direction: 'ltr' }], children: [_jsx(Text, { numberOfLines: 1, style: [
                                            styles.title,
                                            ...
                                            bodyStyle,
                                        ], children: event.body })) : null] })] }) }), actions && actions.length > 0 ? (_jsx(View, { style: [styles.actionsRow, { flexDirection: rtl ? 'row-reverse' : 'row' }], children: actions.map((action) => (_jsx(Pressable, { onPress: action.onPress, accessibilityRole: "button", style: ({ pressed }) => [
                            styles.actionButton,
                            { backgroundColor: colors.accent },
                            pressed ? styles.actionButtonPressed : null,
                        ], children: _jsx(Text, { style: [styles.actionLabel, { color: colors.onAccent }], children: action.label }) }, action.label))) })) : null] }) }));
}
