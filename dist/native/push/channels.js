...
export async function ensureChannels(categories) {
    if (Platform.OS !== 'android')
        return;
    await Promise.all(categories.map((category) => Notifications.setNotificationChannelAsync(category.id, {
 ...
 * under that channel. */
export async function deleteChannel(categoryId) {
    if (Platform.OS !== 'android')
        return;
