...
export function gregorianLeapYear(year: number): boolean {
  return (
    mod(year, 4) === 0 && mod(year, 400) !== 100 && mod(year, 400) !== 200 && mod(year, 400) !== 300
  );
}
...
export function fixedFromGregorian(year: number, month: number, day: number): number {
  ...
  if (month > 2) fixed -= gregorianLeapYear(year) ? 1 : 2;
  return fixed + day;
}

/** Proleptic Gregorian year of a fixed day. */
export function gregorianYearFromFixed(fixed: number): number {
  const d0 = fixed - 1;
...
export function gregorianFromFixed(fixed: number): { year: number; month: number; day: number } {
