...
function decodeGreg(anchorKey: string): { year: number; month: number } {
  const [y, m] = anchorKey.split('-').map(Number);
  return { year: y, month: m };
}
/** Decode a Hebrew anchor key "hy-hm". */
function decodeHeb(anchorKey: string): { year: number; month: number } {
  const [y, m] = anchorKey.split('-').map(Number);
  return { year: y, month: m };
}

function createGregorianLike(opts: CalendarSystemOptions, mixed: boolean): CalendarSystem {
  ...
  return {
    ...
    dayLabels(_page, dayNum, date) {
      if (!mixed) return { big: String(dayNum), sub: null };
    ...
    title(page) {
      if (monthLabelStyle === 'number') return `${page.month + 1}/${page.year}`;
      return `${t(`cal_month_${page.month + 1}`)} ${page.year}`;
    },
  };
}

function createHebrew(opts: CalendarSystemOptions): CalendarSystem {
  ...
  return {
    ...
    monthMarker(page) {
      const hm = page.hebMonth as number;
      const hy = page.hebYear as number;
      return layoutRTL ? hebrewMonthName(hm, hy) : t(hebrewMonthKey(hm, hy));
    },
    title(page) {
      const hm = page.hebMonth as number;
      const hy = page.hebYear as number;
      if (layoutRTL) return `${hebrewMonthName(hm, hy)} ${hebrewYearLetters(hy)}`;
 ...
 */
export function createCalendarSystem(opts: CalendarSystemOptions): CalendarSystem {
  if (opts.type === 'hebrew') return createHebrew(opts);
 ...
 */
export function computeDroppedWeeks(
  ...
  if (system.type === 'hebrew') {
    ...
    if (was.year === now.year && was.month === now.month) return null;
  ...
  if (prev.getFullYear() === now.getFullYear() && prev.getMonth() === now.getMonth()) return null;
  const oldestFlat = prev.getMonth() - back;
  return getMonthGrid(
    prev.getFullYear() + Math.floor(oldestFlat / 12),
    ((oldestFlat % 12) + 12) % 12,
    weekStartsOn,
  ).weeksCount;
}
