 ...
 */
export function hebrewLeapYear(year: number): boolean {
...
export function lastDayOfHebrewMonth(year: number, month: number): number {
  if (month === IYAR || month === 4 || month === ELUL || month === 10 || month === ADAR_II)
    return 29;
  if (month === ADAR && !hebrewLeapYear(year)) return 29;
  const length = daysInHebrewYear(year);
  if (month === CHESHVAN && length !== 355 && length !== 385) return 29;
  if (month === KISLEV && (length === 353 || length === 383)) return 29;
  return 30;
}
 ...
 */
export function hebrewMonthsInOrder(year: number): number[] {
  const out: number[] = [];
  const last = lastMonthOfHebrewYear(year);
  for (let m = TISHREI; m <= last; m++) out.push(m);
  for (let m = NISAN; m <= ELUL; m++) out.push(m);
  return out;
}
 ...
 * @param month 1..13, Nisan-based
 */
export function hebrewMonthOrdinal(year: number, month: number): number {
...
export function fixedFromHebrew(year: number, month: number, day: number): number {
  let fixed = hebrewNewYear(year) + day - 1;
  if (month < TISHREI) {
    const last = lastMonthOfHebrewYear(year);
    for (let m = TISHREI; m <= last; m++) fixed += lastDayOfHebrewMonth(year, m);
    for (let m = NISAN; m < month; m++) fixed += lastDayOfHebrewMonth(year, m);
  } else {
    for (let m = TISHREI; m < month; m++) fixed += lastDayOfHebrewMonth(year, m);
  }
  return fixed;
}

/** Hebrew date of a fixed day. */
export function hebrewFromFixed(fixed: number): HebrewDate {
  ...
  let month = start;
  while (fixed > fixedFromHebrew(year, month, lastDayOfHebrewMonth(year, month))) month++;
...
export function hebrewMonthName(month: number, year: number): string {
  if (month === ADAR) return hebrewLeapYear(year) ? 'אדר א׳' : 'אדר';
 ...
 */
export function hebrewMonthKey(month: number, year: number): string {
  if (month === ADAR && hebrewLeapYear(year)) return 'hebrew_month_12_leap';
  return `hebrew_month_${month}`;
}

const HEBREW_LETTERS_HUNDREDS: Record<number, string> = {
  100: 'ק',
  200: 'ר',
  300: 'ש',
  400: 'ת',
...
const HEBREW_LETTERS_TENS: Record<number, string> = {
  10: 'י',
  20: 'כ',
  30: 'ל',
  40: 'מ',
...
const HEBREW_LETTERS_ONES: Record<number, string> = {
  1: 'א',
  2: 'ב',
  3: 'ג',
  4: 'ד',
...
export function hebrewDayLetters(day: number): string {
  let letters: string;
  if (day === 15) letters = 'טו';
  else if (day === 16) letters = 'טז';
  else {
