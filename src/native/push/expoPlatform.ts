...
function rawFromExpo(notification: Notifications.Notification): RawNotification {
...
function rawResponseFromExpo(response: Notifications.NotificationResponse): RawNotificationResponse {
...
let installedForegroundHandler: PlatformForegroundHandler | null = null;

export function createExpoPlatform(): PushPlatform {
  return {
    os: Platform.OS,
    ...
    setForegroundHandler(handler) {
      ...
      return () => {
        if (installedForegroundHandler !== handler) return;
    ...
    addResponseListener(listener) {
      const subscription = Notifications.addNotificationResponseReceivedListener((response) =>
        listener(rawResponseFromExpo(response)),
      );
      return () => subscription.remove();
    },
...
export function createExpoPushClient(
  options: Omit<PushClientOptions, 'platform'> = {},
