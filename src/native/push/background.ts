...
const COMPLETION_BUDGET_MS = 25_000;

let currentHandler: ((event: NotificationEvent) => Promise<void>) | null = null;
 ...
 */
function toBackgroundEvent(payload: Notifications.NotificationTaskPayload): NotificationEvent | null {
  if (isNotificationResponse(payload)) return null;
  if (payload.notification != null) return null;
...
TaskManager.defineTask<Notifications.NotificationTaskPayload>(BACKGROUND_TASK_NAME, async ({ data, error }) => {
  if (error || !currentHandler) return;
 ...
 */
export function setBackgroundHandler(handler: (event: NotificationEvent) => Promise<void>): () => void {
